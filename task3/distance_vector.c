#include <stdio.h>
#include <stdlib.h>
#include "routing.h" // Include the header file

// Defines the constant LINKCHANGES. If set to 0, link changes are not tracked or allowed. If set to 1, link changes are tracked or handled.
...
// Use structs already defined in routing.h
...
// Function implementations - the declarations are already in routing.h
...
float jimsrand(void) 
{
  // Fix for the random number generation issue
  double mmm = RAND_MAX;   /* largest int as defined in <stdlib.h> */
  float x;                
  x = rand()/(float)mmm;   /* Cast to float for proper division */
  return(x);
}  
